...
The server exposes a small Jev-compatible HTTP API. It never generates free
text: every answer is a structured type extracted from the model's logits in a
single forward pass. This guide documents the routes, the request/response
contract and ready-to-run examples.
...
- **`choice`** — winning label in `choice`, distribution in `probabilities` and
  `confidence`.
- **`score`** — expected value over the levels in `score`, index-to-name legend
  in `legend`, distribution in `probabilities` and `confidence`.
...
- `/health` → `{"status": "ok", "startup_seconds": 12.3}` (model load time).
- `/health/live` → `{"status": "ok"}` and does not depend on the model.
...
- [Training and quantization](training.md) — produce the adapter or quantized
  artifact this API can serve.
