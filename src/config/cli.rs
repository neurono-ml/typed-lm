...
use crate::infrastructure::device::ModelDtype;
use crate::infrastructure::session_cache::{
    DEFAULT_SESSION_CACHE_ENTRIES, DEFAULT_SESSION_CACHE_TOKENS,
};

/// Manaca Jev-like: deterministic evaluation via Candle, Jev-format HTTP API.
#[derive(Parser, Debug)]
#[command(
...
pub enum Command {
    /// Serves the Jev-format HTTP API (POST /v1/systemone).
    Serve(ServeArgs),
}
...
impl ServerBinding {
    pub fn from_serve_arguments(arguments: &ServeArgs) -> Self {
        Self {
...
mod tests {
    ...
    fn sample_serve_arguments(host: &str, port: u16) -> ServeArgs {
        ServeArgs {
            host: host.to_string(),
            port,
            ...
            config_file: None,
            context_path: None,
            served_model_name: "manaca-test-model".to_string(),
    ...
    fn model_dtype_defaults_to_auto_and_parses_overrides() -> anyhow::Result<()> {
        let default_cli = Cli::try_parse_from(["manaca-jev-like", "serve"])?;
        let Command::Serve(default_arguments) = default_cli.command;
        assert_eq!(default_arguments.model_dtype, ModelDtype::Auto);

        let overridden_cli =
            Cli::try_parse_from(["manaca-jev-like", "serve", "--model-dtype", "bf16"])?;
        let Command::Serve(overridden_arguments) = overridden_cli.command;
        assert_eq!(overridden_arguments.model_dtype, ModelDtype::Bf16);
        Ok(())
    }
}
